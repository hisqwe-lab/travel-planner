...
const BUILT_IN_FIREBASE_URL = window.TRAVEL_PLANNER_FIREBASE_URL || "https://honneymoon-266f2-default-rtdb.firebaseio.com/";
const BUILT_IN_TRIP_CODE = window.TRAVEL_PLANNER_TRIP_CODE || "trip-dd23b316-mquneuch";
const BUILT_IN_PUBLIC_APP_URL = window.TRAVEL_PLANNER_PUBLIC_APP_URL || "https://hisqwe-lab.github.io/travel-planner/";
...
const syncState = {
  firebaseUrl: BUILT_IN_FIREBASE_URL || localStorage.getItem(FIREBASE_URL_STORAGE) || "",
  tripCode: BUILT_IN_TRIP_CODE || localStorage.getItem(TRIP_CODE_STORAGE) || "",
  timer: null,
  stream: null,
  saving: false,
  lastRemoteUpdatedAt: "",
  lastSerialized: "",
  publicAppUrl: BUILT_IN_PUBLIC_APP_URL || localStorage.getItem(PUBLIC_APP_URL_STORAGE) || ""
};
...
async function pushRemoteState() {
  ...
  try {
    ...
    setSaveStatus("공동 편집 저장 완료");
  } catch {
    setSaveStatus("공동 편집 저장에 실패했습니다. Firebase 연결을 확인해주세요.");
  } finally {
...
function applyRemoteState(remote, message = "공동 편집 내용 동기화 완료", options = {}) {
  ...
  if (remoteSerialized === syncState.lastSerialized || (!options.force && remoteUpdatedAt < localUpdatedAt)) {
    return false;
  }
...
async function pullRemoteState(options = {}) {
  ...
  try {
    ...
    applyRemoteState(remote, options.message || "공동 편집 내용 동기화 완료", options);
  } catch {
    setSaveStatus("공동 편집 연결을 확인해주세요.");
  }
}
...
function startSync() {
  ...
  setSaveStatus("공동 편집 방에 연결 중입니다.");
  pullRemoteState({ force: true, message: "공동 편집 방에 연결했습니다." });
